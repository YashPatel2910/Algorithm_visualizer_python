...
def draw(draw_info, algo_name, ascending):
    ...
    controls = draw_info.FONT.render(
        "R - Reset | SPACE - Start Sorting | P - Pause | A - Ascending | D - Descending",
        50,
        draw_info.BLACK,
    )
    # for placing the controls
    ...
    sorting = draw_info.FONT.render(
        "I - Insertion Sort | B - Bubble Sort | S - Selection Sort | M - Merge Sort | Q - Quick Sort",
        50,
        draw_info.BLACK,
    )
    # for placing the controls
...
def draw_list(draw_info, color_positions={}, clear_bg=False):
    ...
    if clear_bg:
        clear_rect = (
            ...
            draw_info.TOP_PAD,
            (draw_info.width + 4) - draw_info.SIDE_PAD,
...
def selection_sort(draw_info, ascending=True):
    ...
    for i in range(len(lst) - 1):
        ...
        yield True  # to return control to the calling loop so we can give another input if we want to
...
def merge_sort(draw_info, ascending=True):
    arr = draw_info.lst
    # Create a list of single-element sublist
    sublists = [[num] for num in arr]

    # Merge adjacent sublist until there is only one left
...
def main():
    ...
    while run:
        clock.tick(
            8
        )  # to change the speed of sorting and controls on top of the window

        if (
            sorting
        ):  # execution flow comes here when 'yield' is encountered in sorting function
            try:
